...
public class QuoteService {
    ...
    public Page<QuoteDTO> searchQuotesByText(String text, Pageable pageable) {
        ...
        return quoteRepository.findByTextContainingIgnoreCase(text, pageable)
                .map(quoteMapper::toDto);
    }
    ...
    public Page<QuoteDTO> filterQuotesByCategory(String category, Pageable pageable) {
        ...
        return quoteRepository.findByCategoryIgnoreCase(category, pageable)
                .map(quoteMapper::toDto);
    }
    ...
    public Page<QuoteDTO> searchQuotesByAuthorName(String authorName, Pageable pageable) {
        ...
        return quoteRepository.findByAuthorNameContaining(authorName, pageable)
                .map(quoteMapper::toDto);
    }
    ...
    public Page<QuoteDTO> searchQuotes(String searchTerm, Pageable pageable) {
        ...
        return quoteRepository.searchQuotes(searchTerm, pageable)
                .map(quoteMapper::toDto);
    }
     ...
     */
    public Page<QuoteDTO> filterQuotes(Long authorId, String category, String searchTerm, Pageable pageable) {
        log.debug("Filtering quotes with authorId: {}, category: {}, searchTerm: {}", authorId, category, searchTerm);
        ...
        return quoteRepository.findWithFilters(authorId, category, searchTerm, pageable)
                .map(quoteMapper::toDto);
    }
    ...
    public QuoteDTO createQuote(QuoteDTO quoteDTO) {
        ...
        // Verify author exists
        Author author = authorRepository.findById(quoteDTO.authorId())
    ...
    public QuoteDTO updateQuote(Long id, QuoteDTO quoteDTO) {
        ...
        if (quoteDTO.authorId() != null && !quoteDTO.authorId().equals(existingQuote.getAuthor().getId())) {
            Author newAuthor = authorRepository.findById(quoteDTO.authorId())
                    .orElseThrow(() -> new ResourceNotFoundException("Author", quoteDTO.authorId()));
