...
/**
 * REST Controller for Quote-related endpoints. Provides CRUD operations with validation,
...
public class QuoteController {
  ...
  public ResponseEntity<Page<QuoteDTO>> getAllQuotes(
    ...
    log.info(
        "GET /api/v1/quotes - page: {}, size: {}, sortBy: {}, direction: {}",
        page,
        size,
        sortBy,
        direction);
  ...
  public ResponseEntity<QuoteDTO> getQuoteById(
      @Parameter(description = "Quote ID", required = true) @PathVariable Long id) {
  ...
  public ResponseEntity<Page<QuoteDTO>> searchQuotes(
      @Parameter(description = "Search term (searches in text and author name)")
          ...
          String q,
      @Parameter(description = "Search in quote text only") @RequestParam(required = false)
          String text,
      @Parameter(description = "Search by author name") @RequestParam(required = false)
          String author,
      @Parameter(description = "Page number (0-based)") @RequestParam(defaultValue = "0") @Min(0)
          int page,
      @Parameter(description = "Page size") @RequestParam(defaultValue = "10") @Min(1) @Max(100)
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "404",
            description = "Author not found (if authorId is provided)",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
      })
  @GetMapping("/filter")
  public ResponseEntity<Page<QuoteDTO>> filterQuotes(
      ...
      @Parameter(description = "Filter by category") @RequestParam(required = false)
          String category,
      @Parameter(description = "Search term in quote text") @RequestParam(required = false)
          String searchTerm,
      @Parameter(description = "Page number (0-based)") @RequestParam(defaultValue = "0") @Min(0)
          int page,
      @Parameter(description = "Page size") @RequestParam(defaultValue = "10") @Min(1) @Max(100)
      ...
      @Parameter(description = "Sort direction") @RequestParam(defaultValue = "DESC")
          Sort.Direction direction) {
    log.info(
        "GET /api/v1/quotes/filter - authorId: {}, category: {}, searchTerm: {}",
        authorId,
        category,
        searchTerm);
  ...
  @Operation(
      ...
      description = "Retrieve a list of all distinct quote categories")
  @ApiResponses(
      value = {
  ...
  public ResponseEntity<QuoteDTO> createQuote(
      @Parameter(description = "Quote data", required = true) @Valid @RequestBody
          QuoteDTO quoteDTO) {
  ...
  @ApiResponses(
      value = {
        ...
        @ApiResponse(
            responseCode = "404",
            description = "Quote or Author not found",
            content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
      })
  @PutMapping("/{id}")
  ...
  @Operation(
      ...
      description = "Update specific fields of a quote")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  public ResponseEntity<QuoteDTO> patchQuote(
      @Parameter(description = "Quote ID", required = true) @PathVariable Long id,
      @Parameter(description = "Quote data (partial)", required = true) @RequestBody
          QuoteDTO quoteDTO) {
